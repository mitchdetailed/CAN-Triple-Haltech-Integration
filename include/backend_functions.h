...
typedef enum { CAN_1 = 1 << 0, CAN_2 = 1 << 1, CAN_3 = 1 << 2 } CAN_Bus;
...
#define CAN_MSG_MAX_DLC                                                        \
  ((CAN1_DATALENGTH > CAN2_DATALENGTH)                                         \
       ? ((CAN1_DATALENGTH > CAN3_DATALENGTH) ? CAN1_DATALENGTH                \
                                              : CAN3_DATALENGTH)               \
       : ((CAN2_DATALENGTH > CAN3_DATALENGTH) ? CAN2_DATALENGTH                \
                                              : CAN3_DATALENGTH))
...
typedef struct {
  ...
  uint8_t dlc; /**< Data length in bytes, the number of valid bytes in the data
                  field. */
...
uint8_t setupCAN_FDbus(CAN_Bus bus, uint32_t mainBitrate, uint32_t dataBitrate,
                       bool bitrateSwitch, CAN_Mode mode);
...
uint8_t send_message(CAN_Bus bus, bool is_extended_id, uint32_t arbitration_id,
                     uint8_t dlc, uint8_t data[CAN_MSG_MAX_DLC]);
void onReceive(CAN_Message);

void trigger_CAN_RX(void);
void trigger_CAN_TX(void);
uint8_t add_to_CAN_RX_Queue(CAN_Bus bus, bool EXT_ID, uint32_t ID, uint8_t DLC,
                            uint8_t rxData[CAN_MSG_MAX_DLC]);
...
float dbc_decode(const uint8_t *data, bool is_signed, bool is_big_endian,
                 uint8_t dbc_start_bit, uint8_t dbc_bit_length, float factor,
                 float offset, uint8_t decimal_places);
double dbc_decode_IEEE754(const uint8_t *data, bool is_double,
                          bool is_big_endian, uint8_t dbc_start_bit,
...
int dbc_encode(uint8_t *data, size_t msg_data_length, datatype_t datatype,
               bool is_big_endian, float scaled_value, uint8_t dbc_start_bit,
...
int dbc_encode_ieee754(uint8_t *data, size_t msg_data_length, bool is_float,
                       ...
                       uint8_t dbc_start_bit, uint8_t dbc_bit_length);
